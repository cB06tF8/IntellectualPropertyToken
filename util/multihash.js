...
function parseContractResponse(response) {
  var digest, hashFunction, size;
  var ctr = 0;
  for (let param of Object.keys(response)) {
    if (ctr == 0) {
      digest = response[param];
    } else if (ctr == 1) {
      hashFunction = parseInt(response[param]);
    } else {
      size = parseInt(response[param]);
    }
    ctr++;
  }  
  return {
